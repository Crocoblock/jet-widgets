	...
	class UI_Iconpicker extends UI_Element implements I_UI {
		...
		public function print_icon_set() {
			...
			foreach ( self::$sets as $set => $data ) {
				...
				printf( '<script> if ( ! window.cherry5IconSets ) { window.cherry5IconSets = {} } window.cherry5IconSets.%1$s = %2$s</script>', $set, $json ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
